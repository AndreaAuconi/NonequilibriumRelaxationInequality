...
class Fields:
    ...
    def __init__(self, l, dl, nonlinearity, n_std, coeff_smoothing, precision_smoothing):
        ...
        #self.domain = in_domain(np.sqrt(squared_modulus (np.array(self.xMatrix, self.yMatrix))))
        #self.decay_numerical = np.power(np.exp(-(self.xx + self.yy)/(2*np.power(l/2,2))),4)
    ...
    def getDisplacementPerturbation(self, p, epsilon, theta, phi_smoothing, domain_fraction): 
        in_domain = lambda x: x < self.l*domain_fraction
        domain = in_domain(np.sqrt(self.xx + self.yy))
        pre_phi = -epsilon * dot_prod(direction_vec (theta), grad_ln (p, self.dl))
        pre_phi = pre_phi * domain
